...
type PackIndex = {
  packs: { id: string; title: string; description: string; file: string; badge?: string }[];
};
type Unit = {
  phoneme: string;
  letter: string;
...
export default function Exercises() {
  ...
  useEffect(() => {
    fetch('/content/exercises/index.json')
      .then((r) => r.json())
          ...
          {packs.map((p) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {p.badge && (
                  <span className="text-xs px-2 py-0.5 rounded-full border">{p.badge}</span>
                )}
              </div>
      ...
      {activePack && (
        <div className="grid gap-4">
          ...
          {units.length === 0 && !error && (
            <div className="opacity-70 text-sm">Brak jednostek w tym pakiecie.</div>
          )}
        </div>
      )}
    </div>
